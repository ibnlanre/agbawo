...
(function (global, factory) {
  ...
  var hydrate = function (_a) {
    var name = _a[0],
      value = _a[1];
    return name.split(/\b\.\b/).reduceRight(function (set, curr) {
      var _a;
  ...
  var reStr = function (re) {
    return "/" + re.source + "/" + (/\w+$/.exec(re) || "");
  };
  var trim = function (value) {
    return value.replace(/"/g, "").substring(4);
  };
  var fnStr = function (fn) {
    return (fn.name ? fn.name + "=" : "") + fn;
  };
    ...
    var eachKey = function (key) {
      ...
      if (!(key in newObj) || descriptor.writable) {
        newObj[key] = Array.isArray(obj[key])
          ? obj[key].map(function (e) {
              return clone(e, cyclic);
            })
  ...
  var create = function (obj) {
    ...
    return value === "ArrayBuffer"
      ...
      : "buffer" in obj
      ? (node ? Buffer.from : new obj.constructor()).apply(void 0, buf)
  ...
  function decycle(base) {
    ...
    var walk = function (current, path) {
      ...
      if (typeof current === "string") modified = escapeSpecialChar(current);
      return modified;
    };
    return {
      legend: legend,
  ...
  var hydrate = function (item) {
    var regex = function (symbol) {
      return new RegExp("\\s*" + symbol + "\\s*");
    };
    var colon = function (item) {
      return item.split(regex(":"));
    };
    var semi = function (item) {
      return item.split(regex(";")).filter(Boolean);
    };
  ...
  var combine = function () {
    ...
    var series = deflate(filter(list, "String").map(hydrate));
  ...
  var index = {
     ...
     * hydrate("margin.top: mt-3; margin.bottom: mb-3")
     */
    hydrate: hydrate,
    /**
